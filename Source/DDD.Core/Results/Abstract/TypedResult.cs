...
public abstract class TypedResult<T> : ResultStatus, ITypedResult<T>
{
    ...
    /// </param>
    protected TypedResult(T output, ResultLayer layer) : base(layer)
    {
        _output = output;
        HasOutput  = true;
    }
    ...
    /// <summary>
    /// Initializes a new failed instance of the <see cref="TypedResult{T}"/> class
    /// with the specified failure type, failed layer, and optional reason.
    /// </summary>
    /// <param name="failureType">
    /// The type of failure.
    /// </param>
    /// <param name="failedLayer">
    /// The layer at which the failure occurred.
    /// </param>
    /// <param name="because">
